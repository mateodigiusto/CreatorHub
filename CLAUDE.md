...
- **Phase 1 — Foundation** (mid-flight). Cloud DB schema, auth scaffolding, ESLint guards, video transcoding pipeline, audit/log layers, RLS regression suite, migration round-trip test. **Schema v39 is live in Supabase project `uegzosghcsphjvmehmld`.** Phase 1 part 2 wires auth/login UI, the localStorage-→-DB profile migration, and the first DB-backed read path.
