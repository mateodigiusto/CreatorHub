...
/**
 * The shared client join-link body — link field + copy + QR + rotate.
 * Used three ways: the client Settings panel (ClientJoinLink), the
 * post-create step of the Add Client dialog, and the per-card Invite
 * dialog. All three hit GET/POST /api/clients/[slug]/invite.
 */
export function ClientInviteLinkInline({ slug }: { slug: string }) {
  ...
  function rotate() {
    startRotate(async () => {
      setError(null);
      try {
        const res = await fetch(`/api/clients/${slug}/invite`, { method: "POST" });
  ...
  if (error && !invite) {
    return <p className="text-[12.5px] text-error py-1">{error}</p>;
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <p className="text-[11.5px] text-muted">
          Reusable — anyone with the link can request access until you rotate it.
        </p>
