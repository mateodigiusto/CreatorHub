...
export function Sidebar({
  ...
  const { role: demoRole } = useDemoTeam();
  ...
  // Managers keep Production + the demo Team page (/team) but lose Integrations,
  // Settings, and the agency staff page (/settings/team) — staff/role management
  // is owner/admin only (spec §2).
  const system = systemForRole(profile?.creatorType).filter((it) =>
    demoRole === "manager"
      ? it.href !== "/integrations" && !it.href.startsWith("/settings")
      : true,
  );
